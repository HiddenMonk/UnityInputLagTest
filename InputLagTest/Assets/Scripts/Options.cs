...
public class Options : MonoBehaviour
{
	...
	void Awake()
	{
		...
		changeScene.onValueChanged.AddListener(ChangeScene);
	...
	{
		//Seems to be a bug where the dropdown sometimes doesnt show unless if we destroy it.
		Transform list = changeScene.transform.Find("Dropdown List");
		if(list) GameObject.Destroy(list.gameObject);

		//For some reason this doesnt always work? So we call again OnSceneLoaded.
		CloseOptions();
	...
	void OpenOptions()
	{
		...
		if(mouseTranslate != null) mouseTranslate.enabled = false;
	}

	void CloseOptions()
	{
		optionsMenu.SetActive(false);
		if(mouseTranslate != null) mouseTranslate.enabled = true;
	}
	...
	public static void SetCursorToCenter()
	{
